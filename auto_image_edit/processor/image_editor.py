...
class ImageEditor(BaseImageProcessor):
    ...
    def __init__(self, config: dict, out_dir, is_debug: bool = False, max_num=None):
        super().__init__(config, out_dir, "image_edit", is_debug, max_num)
        self.image_edit = ImageEditorPrompt(self.llm)

    def load_image_data(self, image_path: Path, image_info: dict, image_base64: Optional[str] = None) -> HumanMessage:
        """重写父类方法以适应特定的文本格式"""
        ...
        return super().load_image_data(image_path, text_content, image_base64)
    ...
    def _process_file(self, img_path: Path, image_base64: str, image_creativity: list, **kwargs) -> Optional[str]:
        """单文件处理逻辑，供多线程调用"""
